...
class GUI_Window(wx.Frame):
    ...
    def Run_Event(self, evt):
        ...
        if self.useLast_Btn.IsChecked()|self.Chq_fields():
            
            BIOSUP.main()
        else:
            self.Status_text_ctrl.AppendText("Select at least 1 chipset and vendor or tick 'Run last config' \n")
    def Chq_fields(self):
        allarr = self.AMD_Chq_List.GetCheckedStrings() + self.Intel_Chq_List.GetCheckedStrings()+ self.Vendor_Chq_List.GetCheckedStrings()
        if not len(allarr) == 0:    
            return True
        else:
            return False
